...
is present), the title (bold, wrapped over multiple rows using the same
greedy word-wrap as the description) below that, the description (non-bold,
wrapped over multiple rows) below that if the description is non-empty (an
empty description renders no row and adds no height), then a footer row with
the category name at the bottom-left and the due date at the bottom-right,
...
fixed: it grows to fit however many rows its title, description, and labels
wrap to at the column's width, so no title, description, or label text is
ever clipped.
